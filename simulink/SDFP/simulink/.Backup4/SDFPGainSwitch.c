 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
 * Created: Mon Feb  1 17:03:21 2010
 * 
 *
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME SDFPGainSwitch
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS          1
/* Input Port  0 */
#define IN_PORT_0_NAME      sd_in
...
/* Output Port  0 */
#define OUT_PORT_0_NAME      gain_out
...
/* Output Port  1 */
#define OUT_PORT_1_NAME      fixpt_out
...
/* Output Port  2 */
#define OUT_PORT_2_NAME      p_gain_out
...
extern void SDFPGainSwitch_Outputs_wrapper(const real32_T *sd_in,
                          uint32_T *gain_out,
                          uint32_T *fixpt_out,
                          real32_T *p_gain_out ,
			      const real_T  *xD,
...
extern void SDFPGainSwitch_Update_wrapper(const real32_T *sd_in,
...
{
    const real32_T   *sd_in  = (const real32_T*) ssGetInputPortSignal(S,0);
    uint32_T        *gain_out  = (uint32_T *)ssGetOutputPortRealSignal(S,0);
    uint32_T        *fixpt_out  = (uint32_T *)ssGetOutputPortRealSignal(S,1);
    real32_T        *p_gain_out  = (real32_T *)ssGetOutputPortRealSignal(S,2);
    const real_T   *xD = ssGetDiscStates(S);
    const int_T   p_width0  = mxGetNumberOfElements(PARAM_DEF0(S));
    const int_T   p_width1  = mxGetNumberOfElements(PARAM_DEF1(S));
    const int_T   p_width2  = mxGetNumberOfElements(PARAM_DEF2(S));
    const real32_T  *gain  = mxGetData(PARAM_DEF0(S));
    const uint32_T  *bitwidth  = mxGetData(PARAM_DEF1(S));
    const uint32_T  *fp_position  = mxGetData(PARAM_DEF2(S));

    SDFPGainSwitch_Outputs_wrapper(sd_in, gain_out, fixpt_out, p_gain_out, xD, gain, p_width0, bitwidth, p_width1, fp_position, p_width2);
}
...
/* Function: mdlUpdate ======================================================
  ...
  static void mdlUpdate(SimStruct *S, int_T tid)
  {
    ...
    SDFPGainSwitch_Update_wrapper(sd_in, gain_out, fixpt_out, p_gain_out,  xD, gain, p_width0, bitwidth, p_width1, fp_position, p_width2);
}
