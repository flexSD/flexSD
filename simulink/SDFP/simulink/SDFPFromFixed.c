...
#define S_FUNCTION_NAME SDFPFromFixed
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS          1
/* Input Port  0 */
#define IN_PORT_0_NAME      fixed_in
...
/* Output Port  0 */
#define OUT_PORT_0_NAME      float_out
...
extern void SDFPFromFixed_Outputs_wrapper(const uint32_T *fixed_in,
                          real32_T *float_out  , 
...
{
    const uint32_T   *fixed_in  = (const uint32_T*) ssGetInputPortSignal(S,0);
    real32_T        *float_out  = (real32_T *)ssGetOutputPortRealSignal(S,0);
    const int_T   p_width0  = mxGetNumberOfElements(PARAM_DEF0(S));
    const int_T   p_width1  = mxGetNumberOfElements(PARAM_DEF1(S));
    const uint32_T  *bitwidth  = (const uint32_T *)mxGetData(PARAM_DEF0(S));
    const uint32_T  *fp_position  = (const uint32_T *)mxGetData(PARAM_DEF1(S));

    SDFPFromFixed_Outputs_wrapper(fixed_in, float_out, bitwidth, p_width0, fp_position, p_width1);
}
