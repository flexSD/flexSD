...
/* Module Version: 3.1 */
/* /usr/local/diamond/1.2/ispfpga/bin/lin/scuba -w -lang verilog -synth synplify -bus_exp 7 -bb -arch mg5a00 -type addsub -width 32 -signed -port overflow -pipeline 0 -e  */
/* Thu Jun  7 09:49:40 2012 */


`timescale 1 ns / 1 ps
module add_sub_32_no_outreg (DataA, DataB, Add_Sub, Result, Overflow);
    ...
    FADSU2 addsub_16 (.A0(scuba_vlo), .A1(Add_Sub), .B0(add_sub_inv), .B1(scuba_vlo), 
        .BCI(co15), .CON(Add_Sub), .BCO(), .S0(co_n_1), .S1());

    FADD2B addsub_o0 (.A0(co_n_1), .A1(DataA[31]), .B0(co_n_1), .B1(t_bn), 
        .CI(scuba_vlo), .COUT(co_n_t), .S0(), .S1(Result[31]));
